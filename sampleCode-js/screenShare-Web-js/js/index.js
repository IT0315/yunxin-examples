let appkey = "fd2e0e70a66a445c4e8695a958eae14a"; // 请输入自己的appkey
let appSecret = "2a5ebd62ba52"; // 请输入自己的appSecret
...
async function join() {
  ...
  if (window.localStorage) {
    window.localStorage.setItem(
      `${localStoragePrefix}channelName`,
      channelName
    );
  }
  uid = $("#uid").val();
  ...
  try {
    await rtc.client.join({
      channelName,
      uid,
      wssArr: $("#isGetwayAddrConf").prop("checked")
        ? $("#getwayAddr").val().split(",")
        : null,
    });
